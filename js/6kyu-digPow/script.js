/* 
설명 : Some numbers have funny properties. For example:
...
function digPow(n, p) {
  let seperateNumber = n.toString().split("");
  let sumSeperateNumber = 0;
  seperateNumber.map((v, idx) => {
    sumSeperateNumber += Math.pow(Number(v), p + idx);
  });
  if (!(sumSeperateNumber % n)) {
    return sumSeperateNumber / n;
  } else {
    return -1;
  }
}
